...
function ajaxfunc(filterValue) {
    $.ajax({
        url: `http://localhost:7072/doctorFilterPatients/${filterValue}`,
        method: 'GET',
...
function updateTableOnSelect(data, table) {
    ...
    data.forEach((item, index) => {
        const actionButtons = `<a href="deletePatient?pid=${item.patientvo.id}"><i class="mdi mdi-delete-sweep" style="color: red; font-size: 1.5rem;"></i></a>`;
